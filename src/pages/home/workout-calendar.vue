...
<template>
  ...
  >
    <view
      class="page-shell calendar-page secondary-page safe-bottom"
      :class="themeStore.themeClass"
    >
      <AppHeader title="训练记录" :subtitle="pageSubtitle" show-back @back="goBack">
        <template #right><view class="analysis btn-press" @tap="openAnalysis">分析</view></template>
      </AppHeader>

      <view class="mode">
        <view
          class="mode__item"
          :class="{ 'mode__item--active': activeMode === 'calendar' }"
          @tap="switchMode('calendar')"
          >日历</view
        >
        <view
          class="mode__item"
          :class="{ 'mode__item--active': activeMode === 'records' }"
          @tap="switchMode('records')"
          >记录</view
        >
      </view>
...
.analysis {
  padding: 14rpx 22rpx;
  border: 1rpx solid rgba(255, 100, 24, 0.28);
  border-radius: 20rpx;
