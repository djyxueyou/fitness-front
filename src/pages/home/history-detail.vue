    ...
    <view class="page-shell safe-bottom">
      ...
      <template v-else-if="detail">
        <view class="history-detail__stats">
          ...
          <view class="glass-card history-detail__stat">
            ...
            <view class="history-detail__stat-value">
              {{ formatWeight(Number(detail.totalVolumeKg || 0), unit, 1) }} {{ unit }}
            </view>
        ...
        <view v-if="detailPrs.length" class="glass-card history-detail__prs">
          <view class="history-detail__section-title">本次 PR · {{ detailPrs.length }}</view>
          <view
            v-for="item in detailPrs"
            :key="`${item.exerciseId}-${item.prType}`"
            class="history-detail__pr"
          >
            <view>
              <view class="history-detail__pr-name">{{ item.exerciseName }}</view>
              <view class="history-detail__pr-type">{{ prTypeLabel(item.prType) }}</view>
            </view>
            <view class="history-detail__pr-value">
              {{ prValueText(item.prType, item.value) }}
            </view>
          </view>
        </view>
