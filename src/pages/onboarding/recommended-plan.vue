...
async function performActivate() {
  ...
  try {
    ...
    setTimeout(() => uni.redirectTo({ url: routes.planActive }), 450)
  } catch (err) {
    ...
    <view class="page-shell recommendation safe-bottom" :class="themeStore.themeClass">
      ...
      <template v-else-if="detail">
        <view class="recommendation__hero">
          ...
          <view class="recommendation__meta"
            >{{ detail.cycleWeeks }} 周 · {{ detail.days.length }} 个训练日</view
          >
          <view class="recommendation__reasons">
            <view
              v-for="reason in onboardingStore.profile.recommendationReasons"
              :key="reason"
              class="recommendation__reason"
            >
        ...
        <view class="recommendation__section">
          ...
          <view class="recommendation__days">
            <view v-for="day in weekOneDays" :key="day.id" class="recommendation__day">
              <view class="recommendation__day-index"
                >周{{ ['日', '一', '二', '三', '四', '五', '六'][day.dayOfWeek] }}</view
              >
              <view>
...
.recommendation {
  ...
  &__hero {
    padding: 34rpx;
    border-radius: 38rpx;
    background: linear-gradient(145deg, var(--app-surface-warm), var(--app-accent-soft));
